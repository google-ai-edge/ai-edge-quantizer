...
This is the whole-graph half of OSCAR (the per-tensor half is
algorithms/uniform_quantize/oscar.py). For every quantization site -- a
float activation tensor feeding FULLY_CONNECTED weights (the only op OSCAR
supports, matching the original prototype) -- it
computes per-input-channel scales s from calibrated activation second
moments and rewrites the model equivalently:

    W_consumer <- W_consumer * s        (per input channel; all consumers)
    x_site     <- x_site * (1/s)        (deployed upstream, see below)

The model function is unchanged (float-exact up to fp32 round-off); only the
rounding-error distribution of subsequent weight quantization changes, and
under dynamic-range quantization the runtime activation quantizer sees
compressed outlier channels (the SmoothQuant mechanism).

Deployment hierarchy per site:
  1. FOLD (zero runtime cost): recursively absorb 1/s into the producer's
     own parameters (FC/conv output channels + bias, norm gamma constants,
     ...), passing through scale-commuting ops. All-or-nothing per site.
  2. MUL insertion (small runtime cost, opt-in via `allow_mul_insertion`):
     insert a per-channel constant MUL on the edges into the site's weight
     ops only. Used when no fold path exists and the objective improvement
     clears `mul_min_gain`.
  3. Skip: the site keeps plain (clip-only) OSCAR behavior.

Usage:
    qt = quantizer.Quantizer(float_model_path)
    qt.update_quantization_recipe(..., algorithm_key=AlgorithmName.OSCAR)
    calib = qt.calibrate(calibration_data)           # collects mu2 QSVs
    conditioned, report = oscar_conditioner.condition_model(
        float_model_path, calib, allow_mul_insertion=True)
    print(report.summary())
    oscar_conditioner.verify_conditioned_model(     # mandatory gate
        float_model_path, conditioned, verification_data)
    qt2 = quantizer.Quantizer(conditioned)           # same recipe, quantize
    qt2.load_quantization_recipe(qt.get_quantization_recipe())
    result = qt2.quantize(qt2.calibrate(calibration_data))

Serialization: fold-only conditioning (the default) never re-serializes
the model. All fold edits change buffer *contents* only -- sizes and graph
structure are untouched -- so the output is the original serialized bytes
with the edited buffers patched in place. This sidesteps the flatbuffer
2 GB builder limit entirely and works with both inline and external
(offset/size) buffer layouts. Only `allow_mul_insertion=True` sites force a
re-serialization, which then goes through AEQ's external-buffer big-model
format.
"""
...
# ============================================================================
# MUL insertion (fallback deployment for unfoldable sites)
# ============================================================================


def _insert_scale_mul(
    model, subgraph, site_tensor_idx: int, consumer_ops, inv_s: np.ndarray
) -> None:
  """Inserts a scale MUL operator upstream of consumer operators."""
  site_tensor = subgraph.tensors[site_tensor_idx]
  scale_tensor_id = transformation_utils.add_new_constant_tensor(
      site_tensor.name + b"_oscar_inv_s",
      inv_s.astype(np.float32),
      qtyping.TensorType.FLOAT32,
      subgraph,
      model,
  )
  new_act_id = transformation_utils.add_new_activation_tensor(
      site_tensor.name + b"_oscar_scaled",
      list(
          site_tensor.shapeSignature
          if site_tensor.shapeSignature is not None
          else site_tensor.shape
      ),
      qtyping.TensorType.FLOAT32,
      subgraph,
  )
  mul_op = qtyping.OperatorT()
  mul_op.opcodeIndex = transformation_utils.add_op_code(
      _OP.MUL, model.operatorCodes
  )
  mul_op.inputs = [site_tensor_idx, scale_tensor_id]
  mul_op.outputs = [new_act_id]
  mul_op.builtinOptionsType = qtyping.BuiltinOptions.MulOptions
  mul_op.builtinOptions = flatbuffer_utils.schema_fb.MulOptionsT()

  consumer_indices = []
  for consumer in consumer_ops:
    consumer.inputs[0] = new_act_id
    consumer_indices.append(subgraph.operators.index(consumer))

  insert_at = min(consumer_indices)
  subgraph.operators.insert(insert_at, mul_op)
...
def _skip(site_result: SiteResult, reason: str) -> None:
  site_result.reason = reason
  logging.info(
      "OSCAR conditioner: skipping site %s (%s)",
      site_result.tensor_name,
      reason,
  )
...
def condition_model(
    ...
    *,
    ...
    block_size: int = 0,
    iters: int = 3,
    allow_mul_insertion: bool = False,
    mul_min_gain: float = 1.2,
    min_input_channels: int = 4,
) -> tuple[bytearray, ConditioningReport]:
  """Applies OSCAR scale conditioning to a float model."""
  del num_bits
  ...
  for sg_idx, subgraph in enumerate(model.subgraphs):
    ...
    for tensor_idx in sorted(sites):
      ...
      if weight_buffers in shared_sites:
        status, stored_inv_s, _ = shared_sites[weight_buffers]
        if status == SiteStatus.MUL_INSERTED:
          pending_mul_insertions.append(
              (
                  subgraph,
                  tensor_idx,
                  [subgraph.operators[i] for i in site.consumer_op_indices],
                  stored_inv_s,
              )
          )
          site_result.status = SiteStatus.MUL_INSERTED
          site_result.reason = (
              "shared weight buffer: replicated active MUL insertion"
          )
        elif status == SiteStatus.FOLDED:
          site_result.status = SiteStatus.FOLDED
          site_result.reason = (
              "shared weight buffer: upstream fold already applied globally"
          )
        else:
          _skip(site_result, "shared weight buffer: upstream was skipped")
        continue
      elif any(
      ...
      if mu2 is None:
        _skip(
            site_result,
            "no mu2 in calibration result -- calibrate with the OSCAR"
            " algorithm",
        )
        continue
      ...
      if s is None:
        _skip(site_result, "scaling not beneficial at this site")
        continue
      ...
      if allow_mul_insertion and ratio >= mul_min_gain:
        _scale_consumer_weights(model, subgraph, site, s, patches)
        for buffer_idx in weight_buffers:
          _add_claim(claimed, buffer_idx, "consumer_weights")
        pending_mul_insertions.append(
        ...
        shared_sites[weight_buffers] = (SiteStatus.MUL_INSERTED, inv_s, s)
        site_result.status = SiteStatus.MUL_INSERTED
        site_result.reason = f"fold failed: {fold_error}"
      else:
        shared_sites[weight_buffers] = (SiteStatus.SKIPPED, inv_s, s)
        why = (
            "MUL fallback disabled"
            if not allow_mul_insertion
            else f"gain {ratio:.2f}x below mul_min_gain {mul_min_gain}"
        )
        _skip(site_result, f"fold failed: {fold_error}; {why}")

  for subgraph, tensor_idx, consumer_ops, inv_s in pending_mul_insertions:
    _insert_scale_mul(model, subgraph, tensor_idx, consumer_ops, inv_s)

  _check_cross_subgraph_consistency(model, claimed)

  if not pending_mul_insertions:
    report.in_place_patched = True
    return _patch_serialized_model(model_bytes, patches), report
  return _serialize_model(model), report


def _patch_serialized_model(
    model_bytes: bytearray, patches: dict[tuple[int, int], np.ndarray]
) -> bytearray:
  """Patches constant tensor buffers directly in serialized flatbuffer bytes."""
  ...
  for (sg_idx, tensor_idx), new_data in patches.items():
    ...
    if packed_buffer.Offset():
      start, size = packed_buffer.Offset(), packed_buffer.Size()
      if size != new_data.size:
        raise ConditioningError(
            f"patch size mismatch for tensor {tensor_idx}"
        )
      model_bytes[start : start + size] = new_data.tobytes()
    else:
      view = packed_buffer.DataAsNumpy()
      if not isinstance(view, np.ndarray) or view.size != new_data.size:
        raise ConditioningError(
            f"patch size mismatch for tensor {tensor_idx}"
        )
      view[:] = new_data
...
def _serialize_model(model: qtyping.ModelT) -> bytearray:
  """Serializes model object with external buffer support for large models."""
  # pylint: disable=protected-access
  packed_buffer_data = model_modifier._PackedBufferData(model)
  if packed_buffer_data.packed_size < 256 * 1024:
    return flatbuffer_utils.convert_object_to_bytearray(model)
  return bytearray(
      model_modifier.ModelModifier(model)._serialize_model(
          model, packed_buffer_data
      )
  )
