...
def _get_quant_parameters(quantized_model, signature_data):
...
class CalibrationUtilsTest(parameterized.TestCase):
  ...
  def test_calibration_utils_init_fails(self):
    ...
    error_message = f"open failed for {model_path}: No such file or directory"
    with self.assertRaisesWithPredicateMatch(
        tf.errors.NotFoundError, lambda err: error_message in str(err)
    ):
      calibration_utils.CalibrationUtils(model_path)

  def test_calibration_utils_init_succeeds(self):
    model_path = test_utils.get_path_to_datafile(
        "../tests/models/single_add.tflite"
    )
    calib_utils = calibration_utils.CalibrationUtils(model_path)
    self.assertNotEmpty(calib_utils._signature_runners)
    self.assertNotEmpty(calib_utils._constrained_op_list)
  ...
  def test_align_quant_stats_succeeds(self):
    ...
    self.assertFalse(
        (quant_params == quant_params[0]).all()
