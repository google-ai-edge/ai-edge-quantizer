...
from tensorflow.lite.tools import flatbuffer_utils  # pylint: disable=g-direct-tensorflow-importt
...
class CalibrationUtils:
  ...
  def __init__(self, model_path: str):
    ...
    # Make a list of constrained ops. This is used to identify the ops that
    # need to be constrained to the same scale as the input.
    self._build_constrained_op_list()

  def _build_constrained_op_list(self, verbose=False):
    """Builds a list of constrained ops."""
    ...
    def materialize_standard_op_wrapper(
      ...
      if constraint == _OpQuantConstraint.SAME_AS_INPUT_SCALE:
        self._constrained_op_list.append(op_info.op_name)
      return [qtyping.TensorTransformationParams("")] * 2
    ...
    # Do monkey patch to intercept the `materialize_standard_op` function.
    ...
    # Loop to build up a list of `SAME_AS_INPUT_SCALE` constrained ops.
    for op, func in minmax_func_dict.items():
    ...
    if verbose:
      print(f"  Constrained op list: {self._constrained_op_list}")
  ...
  def _search_tensor_by_signature_name(
      self, signature_key, signature_name, verbose=False
  ):
    """Searches for a tensor name by signature name and output.

    Args:
      signature_key: Name of the signature.
      signature_name: Name of the output in the signature.
      verbose: Flag to enable verbose output.

    Returns:
      The name of the tensor found.
    """
    tensor_names = []
    ...
    if signature_name in input_details.keys():
      tensor_names.append(input_details[signature_name]["name"])

    # Search among outputs.
    output_details = self._signature_runners[signature_key].get_output_details()
    if signature_name not in output_details.keys():
      if tensor_names:
        return tensor_names
      else:
        raise ValueError(
            f"Signature {signature_key} does not have output {signature_name}"
        )

    output_tensor_name = output_details[signature_name]["name"]
    if verbose:
      print(f">> Starting tensor name: {output_tensor_name}")

    # Find corresponding input tensor name.
    ...
    # Recursively search the graph for the output tensor name.
  ...
  def _search_reverse_order_recursively(
      self, graph_info, operators, output_tensor_name, indent, verbose=False
  ):
    ...
    while operators:
      ...
      for output_idx in op.outputs:
        if output_tensor_name == graph_info.subgraph_tensors[
            output_idx
        ].name.decode("utf-8"):
          dbg_str = (
          ...
          if op_name not in self._constrained_op_list:
            if verbose:
  ...
  def align_quant_stats(
      self, qsv: dict[str, Any], signature_data: dict[str, list[str]]
  ...
  def update_quant_stats(
      self,
      qsv: dict[str, Any],
      signature_data: dict[str, list[str]],
