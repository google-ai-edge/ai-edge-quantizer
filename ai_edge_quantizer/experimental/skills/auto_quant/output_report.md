...
-   **Result Figures**: Save any generated visualizations or figures in the
    ...
    curve showing all the models you tried (e.g. Size vs SNR or Size vs MSE).
    Additionally, you must run inference on a sample input to generate and save
    the resulting segmentation masks (e.g., baseline vs. quantized outputs) to
    visually demonstrate fidelity.** Provide the python snippet or artifact
...
```text
**Optimization Target Result:** Successfully managed to fit the ISNET model inside a ~40MB envelope without surpassing the acceptable median MSE threshold of 0.001.
...
You must list **EVERY SINGLE recipe** you evaluated during your exploration in this table so the user can see the full Pareto distribution. Be sure to explicitly recommend and highlight the **Top 3** optimal points (Target, Balanced, and Size profiles) clearly among the other intermediate runs.
...
| **dynamic_int8_selective_ver1 (Target)** | 29.5 | 0.0009 | 45.4 | 3.7x | Base: `dynamic_wi8_afp32()`. Override: `regex='.*target_layer_regex.*'` -> `algorithm_key='no_quantize'`. Skipped final output to recover critical bounds |
...
- **dynamic_int4_selective_iter3 (Size)**: To aggressively compress, we used `dynamic_wi8_afp32()` as the base, retained the bypass on `.*troubled_block_regex.*`, and applied an `op_config` override pushing `num_bits: 4` to all other layers.
