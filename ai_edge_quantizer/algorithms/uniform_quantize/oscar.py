...
def get_tensor_quant_params(
  ...
  if mu2 is None:
    raise ValueError(
        "OSCAR: no activation second moments (mu2) found for op"
        f" {op_info.op_name} (index {op_info.subgraph_op_index}); falling back"
        " to activation-agnostic optimal clipping (OCTAV equivalent) for this"
        " op. If the whole graph has no mu2, consider using OCTAV instead."
    )
