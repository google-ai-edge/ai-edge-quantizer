...
"""Implements OSCAR quant params: activation-aware optimal clipping bounds for weight tensors.
...
This module (`oscar_quant_params.py`) implements the always-representable part
of OSCAR: per-(output channel, block) optimal clipping bounds, found by an exact
breakpoint scan (closed form per segment, no grid search). The bounds feed the
standard min/max machinery (`tensor_zp_scale_from_min_max` +
`uniform_quantize`),
so the output is a standard channelwise/blockwise tensor running on existing
kernels at identical latency to min_max.

The OSCAR algorithm is implemented across 3 files:
1. `oscar_calibration.py`: Collects activation statistics (mu2_j = E[x_j^2]).
2. `oscar_quant_params.py` (this file): Computes the optimal clipping bounds
based on the stats.
