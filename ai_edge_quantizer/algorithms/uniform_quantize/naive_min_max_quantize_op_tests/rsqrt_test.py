...
class RsqrtTest(naive_min_max_test_utils.NaiveMinMaxQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_rsqrt.tflite"
    )
  ...
  def test_materialize_softmax_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.RSQRT,
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_rsqrt,
    )
