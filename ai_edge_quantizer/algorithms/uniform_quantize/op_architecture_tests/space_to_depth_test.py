...
class SpaceToDepthTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = str(
        pathlib.Path(_TEST_DATA_PREFIX_PATH) / "single_space_to_depth.tflite"
    )
  ...
  def test_materialize_space_to_depth_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.SPACE_TO_DEPTH,
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_space_to_depth,
