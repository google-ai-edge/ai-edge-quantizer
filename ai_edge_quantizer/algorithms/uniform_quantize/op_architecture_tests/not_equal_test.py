...
class NotEqualTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_not_equal.tflite"
    )
  ...
  def test_materialize_not_equal_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.NOT_EQUAL,
    ...
    # Test settings.
    op_tensor_names = {}
    op_tensor_names["input"] = "serving_default_input1:0"
    op_tensor_names["input2"] = "serving_default_input2:0"
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_not_equal,
        get_tensor_quant_params_func,
        outputs_to_ignore=[0],  # Bool output does not need to be quantized.
    )
