...
class ResizeNearestNeighborTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_resize_nearest_neighbor.tflite"
    )
  ...
  def test_materialize_resize_nearest_neighbor_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.RESIZE_NEAREST_NEIGHBOR,
