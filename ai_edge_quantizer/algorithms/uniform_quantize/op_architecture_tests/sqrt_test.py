...
class SqrtTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_sqrt.tflite"
    )
    self._op_test_info = op_test_utils.OpTestInfo(
        test_model=tfl_flatbuffer_utils.read_model(self._test_model_path),
        op_tensor_names={},
        input_range=(np.array([[0.0]]), np.array([[100.0]])),
        output_range=(np.array([[0.0]]), np.array([[10.0]])),
    )
  ...
  def test_materialize_sqrt_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.SQRT,
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_sqrt,
