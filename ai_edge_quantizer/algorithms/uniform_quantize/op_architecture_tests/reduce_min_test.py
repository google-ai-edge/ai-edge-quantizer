...
class ReduceMinTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_reduce_min.tflite"
    )
  ...
  def test_materialize_reduce_min_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.REDUCE_MIN,
    ...
    op_tensor_names["input2"] = "Min/reduction_indices"
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_reduce_min,
        get_tensor_quant_params_func,
        same_input_output_params=True,
        inputs_to_ignore=[1],  # Axes do not need to be quantized.
    )
