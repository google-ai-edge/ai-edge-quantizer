...
class GatherTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_gather.tflite"
    )
  ...
  def test_materialize_gather_succeeds(
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.GATHER,
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_gather,
