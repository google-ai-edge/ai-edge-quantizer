...
from ai_edge_quantizer.algorithms.uniform_quantize import naive_min_max_quantize as min_max
...
class GatherNdWithConstIndicesTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_gather_nd_with_const_indices.tflite"
    )
  ...
  @parameterized.parameters(
      ...
      (min_max.get_tensor_quant_params, 8, True),
      (min_max.get_tensor_quant_params, 8, False),
      (min_max.get_tensor_quant_params, 16, True),
      (octav.get_tensor_quant_params, 8, True),
      (octav.get_tensor_quant_params, 16, True),
  )
  def test_materialize_gather_nd_succeeds(
      self, get_tensor_quant_params_fn, activations_num_bits, symmetric
  ):
    ...
    op_info = qtyping.OpInfo(
        op=op,
        op_name=qtyping.TFLOperationName.GATHER_ND,
    ...
    self._test_no_weights_op(
        ...
        self._graph_info,
        self._op_test_info,
        common_quantize.materialize_gather_nd,
...
class GatherNdWithConstParamsTest(op_test_utils.BaseQuantizeTest):

  def setUp(self):
    super().setUp()
    np.random.seed(666)
    self._test_model_path = os.path.join(
        _TEST_DATA_PREFIX_PATH, "single_gather_nd_with_const_params.tflite"
    )
    self._op_test_info = op_test_utils.OpTestInfo(
        test_model=tfl_flatbuffer_utils.read_model(self._test_model_path),
        op_tensor_names={},
        input_range=(np.array([[0]]), np.array([[2]])),
        output_range=(np.array([[1]]), np.array([[8]])),
    )
    # The test model has one subgraph for now.
    self._graph_info = qtyping.GraphInfo(
        subgraph_tensors=self._op_test_info.test_model.subgraphs[0].tensors,
  ...
  def test_materialize_gather_nd_succeeds(
    ...
    # Test settings.
    op_tensor_names = {}
    op_tensor_names["input"] = "Const"
    op_tensor_names["input2"] = "serving_default_indices:0"
    op_tensor_names["output"] = "PartitionedCall:0"
    self._op_test_info.op_tensor_names = op_tensor_names
    self._test_no_weights_op(
        op_info,
