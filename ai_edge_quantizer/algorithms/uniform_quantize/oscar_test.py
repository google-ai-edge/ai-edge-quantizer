...
class OscarQuantParamsTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(
          ...
          expected_block_size=0, expected_quantized_dimension=0,
      ),
      dict(
          ...
          expected_block_size=32, expected_quantized_dimension=1,
      ),
  )
  ...
  def test_missing_mu2_raises_value_error(self):
    w, _ = self._fc_problem()
    op_n = _op_info(_TFLOpName.FULLY_CONNECTED)
    with self.assertRaisesRegex(
        ValueError, 'OSCAR: no activation second moments'
    ):
      oscar.get_tensor_quant_params(
          op_n,
          _TensorQuantConfig(
