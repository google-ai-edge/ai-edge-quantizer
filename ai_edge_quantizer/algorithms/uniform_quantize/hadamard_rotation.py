...
def _randomize_matrix(
    matrix: np.ndarray, size: int, rng: np.random.Generator
) -> tuple[np.ndarray, np.ndarray]:
  """Generates a random Hadamard matrix of the given size.

  Args:
    matrix: The Hadamard matrix to randomize.
    size: The size of the Hadamard matrix. Should be a power of 2.
    rng: The random number generator to use.

  Returns:
    A tuple containing the Hadamard matrix and its inverse. The randomness is
    implemented by multiplying each column by 1 or -1.
  """
  r = rng.integers(0, 2, size=size) * 2 - 1
  rand_diag = np.diag(r)
  return np.matmul(matrix, rand_diag), r
