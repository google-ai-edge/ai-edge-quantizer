...
class ParamsGeneratorTest(parameterized.TestCase):
  ...
  def test_generate_params_marks_correct_buffers_tensors_for_duplication(
    ...
    # Check that the correct buffer and tensor were identified for duplication.
    expected_buffer_id = 2
    expected_tensor = 'arith.constant'
    self.assertEqual(pg._buffers_to_duplicate, [expected_buffer_id])
    self.assertEqual(pg._tensor_names_to_duplicate, [expected_tensor])
  ...
  def test_generate_params_returns_valid_results_when_multiple_tensor_duplication_for_one_buffer(
    ...
    # Check correct buffer and tensor are identified for duplication.
    expected_buffer_id = 2
    sig1_expected_tensor = 'arith.constant'
    sig2_expected_tensor = 'arith.constant1'
    self.assertEqual(pg._buffers_to_duplicate, [expected_buffer_id])
    self.assertEqual(
        pg._tensor_names_to_duplicate,
        [sig1_expected_tensor, sig2_expected_tensor],
    )
