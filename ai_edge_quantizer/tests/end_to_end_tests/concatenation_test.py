...
def _get_dummy_data(num_inputs, num_samples):
  ...
  for _ in range(num_samples):
    samples.append({
        f'input_{i+1}': _RNG.uniform(size=(1, 10, 20, 30)).astype(np.float32)
...
def _get_calibration_data(num_inputs: int = 2, num_samples: int = 128):
...
class ConcatenationTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.float_model_path = test_utils.get_path_to_datafile(
        '../models/two_inputs_concatenation.tflite'
    )
  ...
  def test_concatenate_model_full_integer(self, recipe_path):
