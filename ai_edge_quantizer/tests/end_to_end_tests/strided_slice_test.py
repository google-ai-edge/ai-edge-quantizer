...
"""E2E tests for the quantizer for model with strided_slice."""
...
class StridedSliceTest(parameterized.TestCase):

  def setUp(self):
    super().setUp()
    self.float_model_path = test_utils.get_path_to_datafile(
        '../models/single_strided_slice.tflite'
    )
  ...
  def test_strided_slice_model_full_integer(self, recipe_path):
