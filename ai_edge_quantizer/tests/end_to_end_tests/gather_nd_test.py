...
class GatherNdTest(test_utils.BaseOpTestCase):

  def setUp(self):
    super().setUp()
    self._op_name = qtyping.TFLOperationName.GATHER_ND
  ...
  def test_static_quantization_accuracy_within_tolerance_with_const_indices(
  ...
  def test_static_quantization_accuracy_within_tolerance_with_const_params(
      self, algorithm_key, activations_num_bits, symmetric
  ):
