...
if [ -z "${RELEASE_VERSION}" ] || [ -z "${LITERT_VERSION}" ]; then
    echo "Error: Failed to read RELEASE_VERSION from ../VERSION, or LITERT_VERSION env var is not set."
    exit 1
fi
...
echo "Building ai-edge-quantizer version ${RELEASE_VERSION} with ai-edge-litert == ${LITERT_VERSION}"
